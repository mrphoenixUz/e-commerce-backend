...
export class UsersService {
  ...
  async create(data: Partial<Users>) {
    const existingUsers = await this.usersRepo.count(); // Count users
  
    const user = this.usersRepo.create({
      ...data,
      role: existingUsers === 0 ? UserRole.ADMIN : UserRole.USER, // First user is admin, others are user
    });
  ...
  async addToFavourites(userId: number, productId: string) {
    ...
    // Prevent duplicates by converting to a Set and back to an array
